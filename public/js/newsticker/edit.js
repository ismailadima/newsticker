...
$('.dropzone').dropzone({
    ...
    url: '/newstickers/store'
});

$('.deleteLineClass').hide();
// if({{session('auto_delete_sess')}})
// var autoDelete = '{{ session('auto_delete_sess') }}'
...
//3 adalah MNCTV
// if(autoDelete == 'Y' && unitId == 3){ 
//     $('.deleteLineClass').show()    
// }
// 2022-07-15, sands, open all
if(autoDelete == 'Y'){ 
    $('.deleteLineClass').show()    
}
...
function update()
{
    ...
    // return false
    $.ajax({
        url: '/newstickers/edit/' + newsticker_id,
        data: formData,
        type: 'POST',
...
function updateLine(contentsArr1, contentsArr2, newstickerId, indexData, typeContent, idTableDelete, timeDeletedValue){
    $.ajax({
        url: '/newstickers/update-line/'+newstickerId,
        type: 'POST',
        cache: false,
        data:{
...
{
    $.ajax({
        url: '/newstickers/delete-line/'+newstickerId,
        type: 'POST',
        data:{
            index: index,
            content: content,
